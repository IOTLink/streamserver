...
func InvokeChaincode(client fab.FabricClient, channel fab.Channel, targets []apitxn.ProposalProcessor,
	...
	start1 := time.Now()
	fmt.Printf("start CreateAndSendTransactionProposal %.2fs elapsed\n", time.Since(start1).Seconds())
	...
	fmt.Printf("end CreateAndSendTransactionProposal %.2fs elapsed\n", time.Since(start1).Seconds())

	if err != nil {
		fmt.Println("CreateAndSendTransactionProposal returned error:", err)
	...
	defer eventHub.UnregisterTxEvent(txID)

	start2 := time.Now()
	fmt.Printf("start CreateAndSendTransaction %.2fs elapsed\n", time.Since(start2).Seconds())
	...
	fmt.Printf("end CreateAndSendTransaction %.2fs elapsed\n", time.Since(start2).Seconds())
