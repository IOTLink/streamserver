...
func (c *Client) SaveUserToStateStore(user fab.User, skipPersistence bool) error {
	...
	if !skipPersistence {
		...
		log.Println("SaveUserToStateStor ==== ", string(data[:]))
		/*
		prikey := user.PrivateKey().SKI()

		key, err := c.cryptoSuite.GetKey(prikey)
		 if key.Private() {
			 pri ,_:= key.Bytes()
			 log.Println("user prikey:",string(pri[:]))
		 } else {
			 log.Println("user pubkey")
		 }
		*/
		if err != nil {
...
func (c *Client) GetPreEnrolledUser(name string)  (fab.User, error) {
	if c.userContext != nil {
		return c.userContext, nil
	}
	if name == "" {
	...
	keyPtah := hex.EncodeToString(userJSON.PrivateKeySKI)
	keyPtah = c.config.KeyStorePath() + "/" + keyPtah + "_sk"
	privateKey, err := fabricCaUtil.ImportBCCSPKeyFromPEM(keyPtah, c.cryptoSuite, true)
	if err != nil {
...
func (c *Client) NewTxnID() (apitxn.TransactionID, error) {
	...
	log.Println("NewTxnID ==============> ",c.userContext.Name(), c.userContext.PrivateKey().SKI())
